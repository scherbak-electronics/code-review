...
define(
    [
        'jquery',
        'Magetop_SocialLogin/js/popup',
        'Magento_Ui/js/modal/modal',
        'mage/dropdown',
        'mage/translate',
        'Magetop_Osc/js/jquery.magnific-popup.min'
    ], function ($, socialpopup, modal, dropdown, $t) {
        "use strict";
        $.widget(
            'magetop.socialpopup', socialpopup, {
                options: {
                    captchaForms: [],
                    captchaClientKey: '',
                    ...
                    window.recaptchaOnload = function () {
                        $.each(
                            self.options.captchaForms, function (form, value) {
                                ...
                                switch (value){
                                    case 'user_login':
                                        target = isInvisible ? 'bnt-social-login-authentication' : 'magetop-g-recaptcha-user_login';
                                        if (isInvisible) {
                                            parameters.callback = self.processLogin.bind(self);
                                        }
                                        break;
                                    case 'user_create':
                                        target = isInvisible ? 'button-create-social' : 'magetop-g-recaptcha-user_create';
                                        if (isInvisible) {
                                            parameters.callback = self.processCreate.bind(self);
                                        }
                                        break;
                                    case 'user_forgotpassword':
                                        target = isInvisible ? 'bnt-social-login-forgot' : 'magetop-g-recaptcha-user_forgotpassword';
                                        if (isInvisible) {
