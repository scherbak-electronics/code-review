...
use Magetop\Osc\Helper\Address as OscHelper;
...
class LayoutProcessor implements LayoutProcessorInterface
{
    ...
    public function process($jsLayout)
    {
        ...
        if (isset($shippingStep['shippingAddress']['children']['shipping-address-fieldset']['children'])) {
            ...
            if ($this->_oscHelper->isEnableAmazonPay()) {
                ...
                $shippingConfig['component']                               = 'Magetop_Osc/js/view/shipping';
                $shippingConfig['children']['customer-email']['component'] = 'Magetop_Osc/js/view/form/element/email';
            }
        ...
        if ($this->_oscHelper->isEnableAmazonPay()) {
            ...
            if ($amazonHelper->isPwaEnabled()) {
                $fields['inline-form-manipulator'] = [
                    'component' => 'Magetop_Osc/js/view/amazon'
                ];
            }
        }
    ...
    private function addAddressOption(&$fields, $fieldPosition, $oscField = [])
    {
        foreach ($fields as $code => &$field) {
            ...
            if ((isset($field['config']['elementTmpl'])
                ...
                $classes            .= ' date';
                $field['component'] = 'Magetop_Osc/js/view/form/element/date';
                $field['options']   = [
                    'changeMonth' => true,
                    'changeYear'  => true,
                    'showOn'      => 'both',
     ...
     * @return $this
     */
    private function rewriteTemplate(&$field, $fieldConfig, $template = 'Magetop_Osc/container/form/element/input')
    {
        $elementTmpl = '';
        if (isset($field['config']['elementTmpl'])) {
        ...
        if (isset($field['type']) && $field['type'] === 'group') {
            foreach ($field['children'] as $key => &$child) {
                ...
                if ($key === 0 &&
                    ...
                    $this->rewriteTemplate($child, $fieldConfig, 'Magetop_Osc/container/form/element/street');
                    continue;
                }
            ...
            if ($this->_oscHelper->isUsedMaterialDesign()) {
                $field['config']['template'] = 'Magetop_Osc/container/form/field';
            }
        }
    ...
    {
        if ($this->_oscHelper->isUsedMaterialDesign()) {
            $fields['country_id']['config']['template'] = 'Magetop_Osc/container/form/field';
            $fields['region_id']['config']['template']  = 'Magetop_Osc/container/form/field';
            foreach ($fields['street']['children'] as $key => $value) {
                $fields['street']['children'][0]['label']                 = $fields['street']['label'];
                $fields['street']['children'][$key]['config']['template'] = 'Magetop_Osc/container/form/field';
            }
            $fields['street']['config']['fieldTemplate'] = 'Magetop_Osc/container/form/field';
