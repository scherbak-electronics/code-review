...
namespace Bss\GroupedProductOption\Override\Model\Product\Option;
...
class SaveHandler extends CoreSaveHandler
{
    ...
    public function execute($entity, $arguments = [])
    {
        ...
        foreach ($this->optionRepository->getProductOptions($entity) as $option) {
            if (!empty($optionIds) && !in_array($option->getOptionId(), $optionIds)) {
    ...
    {
        if ($this->bssHelper->isMagentoVersion('2.4.3')) {
            $isProductHasRelations = $this->isProductHasRelations($product);
        }
        /** @var ProductCustomOptionInterface $option */
        foreach ($options as $option) {
            if ($this->bssHelper->isMagentoVersion('2.4.3')) {
                if (!$isProductHasRelations && $option->getIsRequire() && !$this->bssHelper->getConfig('active')) {
