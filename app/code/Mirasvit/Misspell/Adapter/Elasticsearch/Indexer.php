<?php
/**
 * Mirasvit
 *
 * This source file is subject to the Mirasvit Software License, which is available at https://mirasvit.com/license/.
 * Do not edit or add to this file if you wish to upgrade the to newer versions in the future.
 * If you wish to customize this module for your needs.
 * Please refer to http://www.magentocommerce.com for more information.
 *
 * @category  Mirasvit
 * @package   mirasvit/module-search-ultimate
 * @version   2.0.44
 * @copyright Copyright (C) 2021 Mirasvit (https://mirasvit.com/)
 */



declare(strict_types=1);

namespace Mirasvit\Misspell\Adapter\Elasticsearch;

use Mirasvit\Misspell\Adapter\IndexProvider;
use Magento\Elasticsearch\Model\Adapter\Elasticsearch as ElasticsearchAdapter;

class Indexer
{
    const INDEX_NAME = 'mst_misspell_index';

    private $indexProvider;

    private $adapter;

    public function __construct(
        IndexProvider $indexProvider,
        ElasticsearchAdapter $adapter
    ) {
        $this->indexProvider            = $indexProvider;
        $this->adapter                  = $adapter;
    }

    public function reindex(int $storeId): void
    {
        $this->adapter->cleanIndex($storeId, self::INDEX_NAME);
        $this->adapter->checkIndex($storeId, self::INDEX_NAME, false);
        $this->adapter->updateAlias($storeId, self::INDEX_NAME);

        foreach ($this->indexProvider->getPreparedTextData($storeId) as $documents) {
            $this->adapter->addDocs($documents, $storeId, self::INDEX_NAME);
        }

        $this->indexProvider->dropSuggestionTable();
    }
}
