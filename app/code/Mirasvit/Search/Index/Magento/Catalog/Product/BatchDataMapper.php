    ...
    private function addCategoryData(array $documentData): array
    {
        ...
        foreach ($connection->fetchAll($select) as $row) {
            ...
            if (is_array($documentData[$row['product_id']]['_misc'])) {
                $documentData[$row['product_id']]['_misc'] = implode(' ', mb_strtolower(strip_tags($documentData[$row['product_id']]['_misc'])));
            }
            $documentData[$row['product_id']]['_misc'] .= ' ' . mb_strtolower(strip_tags($row['category']));
        }
    ...
    private function addCustomOptions(array $documentData): array
    {
        ...
        foreach ($connection->fetchAll($select) as $row) {
            ...
            foreach (['title', 'sku'] as $field) {
                 if ($row[$field] !== "") {
                       $documentData[$row['product_id']]['_misc'] .= ' ' . mb_strtolower(strip_tags($row[$field]));
                 }
            }
        }
    ...
    private function addBundledOptions(array $documentData): array
    {
        ...
        foreach ($connection->fetchAll($select) as $row) {
            ...
            if (is_array($documentData[$row['parent_id']]['_misc'])) {
                $documentData[$row['parent_id']]['_misc'] = implode(' ', mb_strtolower(strip_tags($documentData[$row['parent_id']]['_misc'])));
            }

            $documentData[$row['parent_id']]['_misc'] .= $this->configProvider->applyLongTail(mb_strtolower($row['sku']));
            $childrenData = $row['sku'] .' '. $row['varchar_value'] .' '. $row['text_value'] .' '. $row['decimal_value'];
            $childrenData = mb_strtolower(strip_tags($childrenData));
