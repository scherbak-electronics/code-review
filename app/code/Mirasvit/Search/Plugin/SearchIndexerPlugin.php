                ...
                if (!empty($value) && in_array($attribute->getFrontendInput(), ['multiselect', 'select'])) {
                    if (is_array($value)) {
                        foreach ($value as $nestedKey => $nestedValue) {
                            $attributeData[$attributeId][$nestedKey] = strtolower($nestedValue);
                        }
                    } else {
                        $attributeData[$attributeId] = strtolower($value);
                    }
                    continue;
                }

                if (isset($productData[$attributeId])) {
                    $attributeData[$attributeId] = strtolower(trim(strip_tags($productData[$attributeId])));
                }
            }
        }
