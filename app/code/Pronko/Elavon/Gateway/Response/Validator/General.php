...
namespace Pronko\Elavon\Gateway\Response\Validator;
...
/**
 * Class General
 * @package     Pronko\Elavon\Gateway\Response\Validator
 */
class General extends AbstractValidator
{
    ...
    {
        $response = $this->requestSubjectReader->readResponse($validationSubject);
    ...
    private function isHashMatch(array $response)
    {
        if (isset($response['timestamp']) && isset($response['merchantid'])
            ...
            $hashString = $this->hashEncryptor->encrypt([
                ...
                $response['message'],
                $response['pasref'],
                is_array($response['authcode']) ? '' : $response['authcode']
            ]);
