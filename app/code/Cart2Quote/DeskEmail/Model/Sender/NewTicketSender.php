...
/**
 * Class NewTicketSender
 * @package Cart2Quote\DeskEmail\Model\Sender
 */
class NewTicketSender extends Sender
{
    /**
     ...
     * @param \Cart2Quote\DeskEmail\Model\Container\NewTicketIdentity $identityContainer
    ...
    public function __construct(
        ...
        \Cart2Quote\DeskEmail\Model\Container\NewTicketIdentity $identityContainer,
    ...
    protected function checkAndSend(TicketInterface $ticket, MessageInterface $message)
    {
        ...
        if ($ticket->getCustomerName() && $ticket->getCustomerEmail()) {
            $this->prepareTemplate($message, $ticket);
            $this->sendOnce();
        }

        if ($ticket->getAssigneeName() && $ticket->getAssigneeEmail()) {
            $this->prepareTemplateAdmin($message, $ticket);
            $this->sendOnce();
        }
    ...
    /**
     * Prepare the template for the admin
     *
     * @param MessageInterface $message
     * @param TicketInterface $ticket
     * @return $this
     */
    protected function prepareTemplateAdmin(MessageInterface $message, TicketInterface $ticket)
    {
        $this->_templateContainer->setTemplateOptions($this->getTemplateOptions());
