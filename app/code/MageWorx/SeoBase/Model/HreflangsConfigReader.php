...
class HreflangsConfigReader
{
    ...
    public function getHreflangSettings(): array
    {
        ...
        $value                  = $this->scopeConfig->getValue(self::XML_PATH_HREFLANGS_HREFLANG_SETTINGS);
    ...
    protected function getLanguageCodeFromLocale(int $storeId): string
    {
        if ($storeId != $this->storeManager->getStore()->getId()) {
            $locale = $this->configDataLoader->getConfigValue(self::XML_PATH_MAGENTO_LANGUAGE_CODE, $storeId);
        }

        $locale = isset($locale) ? $locale : $this->scopeConfig->getValue(
    ...
    {
        if ($storeId != $this->storeManager->getStore()->getId()) {
            $countryCode = $this->configDataLoader->getConfigValue(self::XML_PATH_MAGENTO_COUNTRY_CODE, $storeId);
        }

        return isset($countryCode) ? $countryCode : $this->scopeConfig->getValue(
