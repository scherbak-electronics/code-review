...
class Main extends GenericForm implements TabInterface
{
    ...
    protected function getIsSingleStoreMode()
    {
        if (is_null($this->getRequest()->getParam('is_new'))) {
            return $this->getCategoryTemplate()->getIsSingleStoreMode();
        }
    ...
    protected function getSwitcherScript()
    {
        ...
        $selector = "'#template_assign_type" . $template->getAssignType() . "'";
        ...
        return
            ...
            $('#template_assign_type1').on('change', function() {
                if ($('#template_assign_type1:checked')) {
                    $('li[aria-labelledby=template_category_tabs_categories]').hide();
                }
            });

            $('#template_assign_type2').on('change', function() {
                if ($('#template_assign_type2:checked')) {
                    $('li[aria-labelledby=template_category_tabs_categories]').show();
                }
            });
    ...
    protected function getLnAllFiltersComment()
    {
        ...
        $string .= "<br>&nbsp;&nbsp;&nbsp;&nbsp;" . __(
                'Example:'
            ) . " <b>" . '[category][ – parameters: {filter_all}]' . "</b>";
        $string .= " - " . __(
                'If "color", "occasion", and "shoe size" attributes are chosen, on the frontend you will see:'
            );
        $string .= "<br>&nbsp;&nbsp;&nbsp;&nbsp;" . __(
                '"Shoes – parameters: Color Red, Occasion Casual, Shoe Size 6.5"'
            );
    ...
    protected function getAdditionalComment()
    {
        ...
        $note .= __(
            'Note: The variables [%s] and [%s] will be replaced by their values Only on the front-end.',
            'filter_all',
            "filter_<i>attribute_code</i>"
        );
        $note .= ' ' . __(
                'So, in the backend you will still see [%s] and [%s].',
                'filter_all',
                "filter_<i>attribute_code</i>"
            );
    ...
    {
        return '<p>' . __(
                'Randomizer feature is available. The construction like %1 will use a randomly picked word.',
                '[Buy||Order||Purchase]'
            ) . '<br>' . __(
                'Also randomizers can be used within other template variables, ex: %1. Number of randomizers blocks is not limited within the template.',
                '[-parameters:||-filters: {filter_all}]'
            ) . '<br>';
    }
}
