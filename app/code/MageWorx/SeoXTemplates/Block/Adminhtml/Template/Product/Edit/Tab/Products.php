...
class Products extends ExtendedGrid implements TabInterface
{
    ...
    protected function getSelectedProducts()
    {
        ...
        if (!empty($params['selected_products']) && is_array($params['selected_products'])) {
            ...
            $selectedProductIds = $this->getProductTemplate()->getProductData();

            if (!is_array($selectedProductIds)) {
                $selectedProductIds = [];
            }

            $selectedProductIds = array_combine(array_values($selectedProductIds), array_values($selectedProductIds));
        }
