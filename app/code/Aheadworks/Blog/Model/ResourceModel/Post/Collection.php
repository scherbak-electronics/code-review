...
class Collection extends \Aheadworks\Blog\Model\ResourceModel\AbstractCollection
{
    ...
    public function addFieldToFilter($field, $condition = null)
    {
        if ($field == 'virtual_status') {
            return $this->addVirtualStatusFilter(array_pop($condition));
        }
        return parent::addFieldToFilter($field, $condition);
    }

    /**
     * Add filter by virtual status
     *
     * @param array $virtualStatus
     * @return $this
     */
    public function addVirtualStatusFilter($virtualStatus)
    {
        if (!is_array($virtualStatus)) {
        ...
        if (in_array(Status::DRAFT, $virtualStatus)) {
            $conditions[] = $conn->quoteInto('status = ?', Status::DRAFT);
        }
        if (in_array(Status::PUBLICATION_PUBLISHED, $virtualStatus)) {
            $conditions[] =
                $conn->quoteInto('status = ?', Status::PUBLICATION)
                . ' AND '
                . $conn->quoteInto('publish_date <= ?', $now);
        }
        if (in_array(Status::PUBLICATION_SCHEDULED, $virtualStatus)) {
            $conditions[] =
                $conn->quoteInto('status = ?', Status::PUBLICATION)
                . ' AND '
                . $conn->quoteInto('publish_date > ?', $now);
