    ...
    protected function preparePostData(array $postData)
    {
        ...
        if ($saveAction = $this->getRequest()->getParam('action')) {
            if ($saveAction == 'save_as_draft') {
                ...
                $postData['virtual_status'] = Status::DRAFT;
            }
            if ($saveAction == 'publish' || $postData['virtual_status'] == Status::PUBLICATION_PUBLISHED) {
                $postData['status'] = Status::PUBLICATION;
                $postData['publish_date'] = $this->dateTime->gmtDate(
                    \Magento\Framework\Stdlib\DateTime::DATETIME_PHP_FORMAT
                );
            }
            if ($saveAction == 'schedule') {
                $publishTimestamp = strtotime($postData['publish_date']);
                $postData['publish_date'] = $this->dateTime->gmtDate(
                    \Magento\Framework\Stdlib\DateTime::DATETIME_PHP_FORMAT,
                    $publishTimestamp
                );
